...
def clv_event_category_export_sqlite_10(client, args, db_path, table_name):
    ...
    clv_event_category = client.model('clv.event.category')
    event_category_browse = clv_event_category.browse(args)
    ...
    for event_category in event_category_browse:
        ...
        code = None
        if event_category.code:
            code = event_category.code

        description = None
        if event_category.description:
...
def clv_event_category_import_sqlite_10(client, args, db_path, table_name):
    ...
    for row in cursor:
        ...
        print(
            event_category_count, row['id'], row['parent_id'], row['name'].encode('utf-8'), row['code']
        )
